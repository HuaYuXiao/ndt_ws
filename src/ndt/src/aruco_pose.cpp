...
ArucoEstimator::ArucoEstimator() : it_(nh_) {
    // Load parameters
    nh_.param<std::string>("camera_topic_prefix", camera_topic_prefix, "/d435/color/");
    ...
    image_pub_ = it_.advertise("/ndt/image_with_marker", 1);
    cam_pos_pub_ = nh_.advertise<geometry_msgs::PoseStamped>("/ndt/aruco_pose/", 10);

    ROS_INFO("Aruco Estimator initialized");
}

void ArucoEstimator::loadCameraParams() {
    std::string config_path = ros::package::getPath("ndt") + 
                            "/config/" + camera_parameters_file;
...
void ArucoEstimator::imageCallback(const sensor_msgs::ImageConstPtr& msg) {
    ...
    if (fractal_detector_.detect(gray)) {
        ...
        if (fractal_detector_.poseEstimation()) {
            ...
            pose.header.frame_id = "d435_color_frame";
