...
class Normal:
    def __init__(self):
        ...
        # 点击后积累帧数设置（默认为150帧）
        self.max_collect_frames = int(rospy.get_param('~collect_frames', 30))
    ...
    def depth_cb(self, msg: Image):
        ...
        try:
            with self.lock:
                ...
                # 根据不同编码格式进行转换
                if '16U' in msg.encoding or '16UC1' in msg.encoding:
                    cv_img = self.bridge.imgmsg_to_cv2(msg, desired_encoding='16UC1')
                elif '32F' in msg.encoding or '32FC1' in msg.encoding:
    ...
    def depth_to_meters(self, depth_patch, encoding):
        ...
        if depth_patch is None or encoding is None:
            return None
        # 16位编码通常以毫米为单位，转换为米
        if '16U' in encoding or '16UC1' in encoding or encoding.lower().startswith('16'):
            depth_m = depth_patch.astype(np.float32) / 1000.0
            depth_m[depth_m == 0] = np.nan  # 0值视为无效
            return depth_m
        else:
            # 32位浮点通常已为米单位
            depth_m = depth_patch.astype(np.float32)
            depth_m[depth_m == 0] = np.nan
            depth_m[np.isinf(depth_m)] = np.nan  # 无穷大视为无效
            return depth_m
    ...
    def fit_plane_svd(self, points):
        ...
        if pts_centered.shape[0] < 3:  # 有效点不足3个
            return None, None
    ...
    def build_frame_axes_from_normal(self, normal):
        ...
        参数:
            normal: 平面法线向量（x轴初始方向）
        返回:
            x_axis, y_axis, z_axis: 三个坐标轴向量
        """
        x_axis = np.array(normal, dtype=np.float64)
        if np.linalg.norm(x_axis) == 0:  # 法线无效
            return None
        x_axis = x_axis / np.linalg.norm(x_axis)  # 归一化x轴

        # 相机水平方向（光学坐标系中+X向右）
        cam_horiz = np.array([1.0, 0.0, 0.0], dtype=np.float64)

        # 将相机水平方向投影到垂直于法线的平面上（得到y轴初始方向）
        proj = cam_horiz - np.dot(cam_horiz, x_axis) * x_axis
        nrm = np.linalg.norm(proj)
        # 如果投影接近零，尝试相机垂直方向（+Y向上）
        if nrm < 1e-3:
            cam_alt = np.array([0.0, 1.0, 0.0], dtype=np.float64)
            proj = cam_alt - np.dot(cam_alt, x_axis) * x_axis
            nrm = np.linalg.norm(proj)
            # 如果仍接近零，尝试其他方向
            if nrm < 1e-3:
                if abs(x_axis[0]) < 0.9:
                ...
                proj = tmp - np.dot(tmp, x_axis) * x_axis
                nrm = np.linalg.norm(proj)
                if nrm < 1e-6:  # 仍无效，返回None
                    return None

        y_axis = proj / nrm  # 归一化y轴
        z_axis = np.cross(x_axis, y_axis)  # z轴为x与y的叉积
        ...
        z_axis = z_axis / np.linalg.norm(z_axis)  # 归一化z轴
    ...
    def compute_and_draw(self):
        ...
        # 若无RGB图像，显示空白窗口
        if rgb is None:
            cv2.imshow(self.window_name, np.zeros((240,424,3), dtype=np.uint8))
            return
        ...
        valid_mask = ~np.isnan(depth_flat) & (depth_flat > 0.001)
        if np.count_nonzero(valid_mask) < 3:  # 有效点不足3个
            print("有效深度点不足")
            cv2.imshow(self.window_name, vis)
            return
        ...
        if self.collecting:
            ...
            if self.frames_collected >= self.max_collect_frames:
                ...
                if not self.pose_published:
                    ...
                    rospy.loginfo("已发布一次最终姿态（%d 帧平均）", self.frames_collected)
