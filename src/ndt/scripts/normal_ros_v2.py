...
class Normal:
    ...
    def build_frame_axes_from_normal(self, normal):
        ...
        """
        # 1. 获取世界坐标系 Z 轴在当前相机坐标系下的向量表示
        try:
            # 假设 camera_init 到 camera_frame 的变换已知
            # 我们需要的是 camera_init 空间下的 (0,0,1) 向量转换到 self.camera_frame
            # 这里使用 lookupTransform 或 transformVector
            # 为了简洁，我们创建一个带时间戳的坐标点进行转换
            up_world = PointStamped()
            up_world.header.frame_id = "camera_init"
            up_world.header.stamp = rospy.Time(0) # 获取最新变换
            up_world.point.x = 0.0
            up_world.point.y = 0.0
            up_world.point.z = 1.0
            
            # 转换到相机光轴坐标系
            up_in_cam_msg = self.tf_listener.transformPoint(self.camera_frame, up_world)
            # 获取相机系下的“上方”向量
            z_axis = np.array([up_in_cam_msg.point.x, up_in_cam_msg.point.y, up_in_cam_msg.point.z])
            z_axis /= np.linalg.norm(z_axis)
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException) as e:
            rospy.logwarn_throttle(5, "TF转换失败，无法确定全局Z轴方向: %s", e)
            return None
        ...
        if angle_deg > 45.0:
            rospy.logwarn("构建的X轴与法线偏角过大 (%.2f° > 30°)，拒绝该姿态", angle_deg)
            return None
